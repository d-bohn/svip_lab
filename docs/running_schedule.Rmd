...
```{r}
# 8 slots
add_dates(start, 7, reps = c(mrep=1,trep=2,wrep=3,ttrep=1,frep=1)) %>% 
  add_times(., '5:00-8:00', '12:00-2:00', '2:00-4:00','9:30-12:00',
            '2:00-5:00', '5:00-7:00','12:00-2:30','9:30-12:00') %>% 
  add_rooms(., 463, 472, 472, 463, 463, 463, 463, 463) %>% 
  add_ras(., Dan, Alyssa, Kelley, Kelley, Alyssa, Dan, Alyssa, Dan) %>% 
  add_studies(., ccae2, ahs_II_472, ahs_II_472, bza_II, ccae2, ccae2, ccae2, bza_II) %>%
  make_table()
```
